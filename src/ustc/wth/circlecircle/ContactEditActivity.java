...
public class ContactEditActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final ContactInfo contactinfo = (ContactInfo) bundle
				.get("ContactInfo1");
		...
		// //int
		// str=Integer.parseInt(bundle.getString("str"));//getString()返回指定key的值
		...
		Cursor groupscursor = getContentResolver().query(
				ContactsContract.Groups.CONTENT_URI, RAW_PROJECTION,
				RAW_CONTACTS_WHERE, new String[] { "" + 0 }, null);
		if (groupscursor != null) {
			while (groupscursor.moveToNext()) {
				String title = groupscursor.getString(groupscursor
						.getColumnIndex("title"));
				String id = groupscursor.getString(groupscursor
						.getColumnIndex("_id"));
		...
		while (groupcursor.moveToNext()) {
			...
			groupid = groupcursor.getString(groupcursor
	...
	private void updateContact(String oldname) {
		...
		ops.add(ContentProviderOperation
				...
				.withValue(Phone.NUMBER, edittext_phone.getText().toString())
				.build());
		...
		ops.add(ContentProviderOperation
				...
				.withValue(Email.DATA, newemail).build());
		// 更新im
		...
		ops.add(ContentProviderOperation
				...
				.withValue(Im.DATA, edittext_im.getText().toString()).build());
		...
		if (personstreet.equals("")) {
			...
			values.put(StructuredPostal.DATA1, edittext_address.getText()
					.toString());
			...
			ops.add(ContentProviderOperation
					...
					.withSelection(
							...
							new String[] { String.valueOf(id),
									...
									String.valueOf(StructuredPostal.TYPE_HOME) })
					.withValue(StructuredPostal.DATA1,
							edittext_address.getText().toString()).build());
		}
		...
		ops.add(ContentProviderOperation
				...
				.withValue(StructuredName.DISPLAY_NAME,
						edittext_name.getText().toString()).build());
		...
		// .withSelection(Data.RAW_CONTACT_ID + "=?" + " AND "+
		// ContactsContract.Data.MIMETYPE + " = ?",new String[] {
		// String.valueOf(id),Note.CONTENT_ITEM_TYPE }).withValue(Note.NOTE,
		// note).build());
	...
	class MyButtonClickListener implements OnClickListener {
		public void onClick(View v) {
			...
			try {
				...
				Toast.makeText(ContactEditActivity.this, "编辑失败 ",
						Toast.LENGTH_SHORT).show();
			}
			Toast.makeText(ContactEditActivity.this, "编辑成功 ",
					Toast.LENGTH_SHORT).show();
